 ...
 * <p>
 * 本クラスのインスタンスでは，
 * 一行ごとのコードマップオブジェクト（CodeLineMapForC インスタンス）をリストとして保持する．
 * 
...
{
    /**
     * C ソースファイルに対応した CodeMap オブジェクトを生成する
     * 
     * @param aSourceFile ソースファイル
     * @throws NotSupportedSourceFileExeption ソースファイルが C ソースでない場合
            ...
            while ( idx < line.length() ){    
                ...
                if ( Character.isWhitespace(ch) ){ 
                    ...
                    if ( status == TRADITIONAL ){ // Traditional コメントの場合 
                        map.append(CodeLineMapForC.TRADITIONAL_COMMENT);
                        contents.append(ch);
                    }
                ...
                if ( status == CODE ){ 
                    if ( ch == '/' ){ // コメント開始の可能性: 先読みして決める
                        ...
                        if ( remainedPart.startsWith("//") ){ // EOL コメントとして行末までを処理
                            ...
                            appendCodeToMap(map, isCommentOut(trimComment(remainedPart)) ? CodeLineMapForC.EOL_COMMENT_OUT : CodeLineMapForC.EOL_COMMENT, remainedPart.length());
                        }
                        else if ( remainedPart.startsWith("/*") ){ // Traditional コメントとして状態遷移
                            status = TRADITIONAL;
                            idx += "/*".length();
                            appendCodeToMap(map, CodeLineMapForC.TRADITIONAL_COMMENT, "/*".length());
                            contents.delete(0, contents.length());
                            contents.append("/*");
                        }
                        else{
                            idx++;
                            appendCodeToMap(map, CodeLineMapForC.CODE, 1);
                        }
                    }
                    else{
                    ...
                    if ( ch == '*' && idx+1 < line.length() && line.charAt(idx+1) == '/' ){
                        ...
                        if ( isCommentOut(trimComment(new String(contents))) ){
                            ...
                            for ( int i = map.length()-1; i >= 0; i-- ){
                                map.replace(i, i+1, CodeLineMapForC.TRADITIONAL_COMMENT_OUT);
                                length--;
                                if ( length == 0 ){
                                    break;
                                }
                        ...
                        idx++;
                        appendCodeToMap(map, CodeLineMapForC.TRADITIONAL_COMMENT, 1);
                        contents.append(ch);
                    }
                    continue;
                }
    ...
    public boolean isCommentOut(final String aFragment)
    {
        ...
        // ならばコードの一部と見なす
        List<String> list = (new CTokenizer(aFragment)).getTokenList();
        if ( list.size() == 0 ){
            return false;
        }
